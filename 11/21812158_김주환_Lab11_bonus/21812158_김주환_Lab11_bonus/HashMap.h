...
public: // public types
	...
	typedef SkipList<K, Entry> Bucket;// a bucket of entries // Entry를 V로 바꾸는 것도 고려할것
...
{
	return *bkt; // ent를 가리킬지 skiplist를 가리킬지 결정해야한다.
}
...
typename HashMap<K, V>::Iterator HashMap<K, V>::_find(const K& k)
{
	...
	pos = bkt.operator*().SkipListSearch(k);
...
typename HashMap<K, V>::Iterator HashMap<K, V>::_insert(const Iterator& p, const Entry& e) {
	p.bkt.operator*().SkipListInsert(e.getKey(), e); // insert before p using insert() of list<Entry>
	num_entry++; // one more entry
	return Iterator(B, p.bkt); // return this position
}
...
void HashMap<K, V>::_erase(const Iterator& p, const K& k) {
	p.bkt->SkipListRemove(k); // remove entry from bucket
