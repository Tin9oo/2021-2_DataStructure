...
void Mtrx::mtrxStats(ostream& fout) {
	int i, j, k;
	double mean = 0.0;
	double sum = 0.0;
	double medi = 0.0;
	...
	Mtrx mR("Mtrx status", n_row + 1, n_col + 1);

	for (i = 0; i < this->n_row; i++) {
		for (j = 0; j < this->n_col; j++) {
			mR.dM[i][j] = this->dM[i][j];
			mR.dM[i][n_col] += this->dM[i][j];
			mR.dM[n_row][n_col] += this->dM[i][j];

			diff_sq_sum +=
				(this->dM[i][j] - mean) * (this->dM[i][j] - mean);
		}
	}
	for (j = 0; j < n_col; j++) {
		for (i = 0; i < n_row; i++) {
			mR.dM[n_row][j] += this->dM[i][j];
		}
	}
	sum = mR.dM[n_row][n_col];
	...
	*this = mR;
	...
	fout << "Mtrx status" << endl;
	fout << "avg : " << mean << ", var : " << var << ", st_dev : " << std_dev << endl;
}